...
def filter_graph_points_24(
    ...
    height_error_threshold=0.10,
    peak_window=2,
):
    """
    Point compression for profile graphs.

    Priority:
    1) start/end points are always kept
    2) slope-change points are always kept
    3) high/low points compared with surrounding nodes are kept
    4) points that would cause vertical interpolation error are kept
    5) max_dist points are only added as low-priority fillers for empty-looking gaps

    Existing qt_app.py calls this function with only:
        filter_graph_points_24(df, log_func, slope_threshold, max_dist)

    So the new parameters above have defaults and do not require qt_app.py changes.
    """
    df = df.copy()
    ...
    # 1) Keep slope-change points and local high/low points first.
    for i in range(1, n - 1):
        slope_in = safe_slope(i - 1, i)
        slope_out = safe_slope(i, i + 1)

        if slope_in is None or slope_out is None:
            # Same distance duplicated or vertical section: shape is ambiguous, keep it.
        ...
        if neighbor_values:
            ...
            # High point compared with nearby nodes.
            peak_prom = y[i] - neighbor_min
            is_local_peak = y[i] >= neighbor_max and peak_prom >= peak_prominence

            # Low point is also shape-critical for the graph line.
            valley_prom = neighbor_max - y[i]
            is_local_valley = y[i] <= neighbor_min and valley_prom >= peak_prominence
    ...
    def add_height_error_points():
        """
        If the current kept points would connect over an actual high/low point,
        add the point with the largest vertical error.
        Repeat until every segment is within height_error_threshold.
        """
        added_any = False
    ...
    # 2) Add points that are required to draw the original height shape.
    add_height_error_points()

    # 3) Add low-priority filler points only when important points are too far apart.
    #    This replaces the old behavior where max_dist competed with slope/peak logic.
    filler_count = 0
    if max_dist > 0:
        while True:
            ordered = sorted(kept, key=lambda idx: x[idx])
            ...
            for left, right in zip(ordered, ordered[1:]):
                ...
                def filler_rank(idx):
                    # Main goal: fill the empty distance.
                    # Tie-breaker: choose a slightly more meaningful shape point.
