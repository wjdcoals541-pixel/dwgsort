...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.dxf_profile_match_df = None
        self.dxf_profile_source_path = None
    ...
    def choose_files(self):
        files, _ = QFileDialog.getOpenFileNames(
            ...
            "도면 파일 선택",
            "",
            "Supported files (*.pdf *.dxf *.xls *.xlsx);;PDF files (*.pdf);;DXF files (*.dxf);;Excel files (*.xls *.xlsx)",
        )
    ...
    def extract_dxf_profile(self, file_path):
        try:
            _text_df, profile_df = process_dxf_profile(
                file_path,
                self.append_log,
            ...
            self.dxf_profile_match_df = None
            self.dxf_profile_source_path = None
            self.append_log(f"[DXF][ERROR] 텍스트 추출 실패: {exc}")
            raise

        self.dxf_profile_match_df = profile_df.copy()
        self.dxf_profile_source_path = file_path
        if profile_df.empty:
            self.last_raw_preview_df = None
            self.last_filtered_df = None
            self.last_saved_path = None
            self.clear_inline_result()
            self.set_preview_message("DXF에서 그래프로 표시할 매칭 결과를 만들지 못했습니다.")
            return False

        self.last_raw_preview_df = profile_df.copy()
        self.preview_source_path = file_path
        self.show_preview_dataframe(self.last_raw_preview_df, "DXF 전체 점 미리보기")
        self.set_status("DXF 추출 결과 준비 완료")
        return True
    ...
    def preview_graph(self):
        ...
        if file_path.lower().endswith((".pdf", ".dxf")):
            if self.last_filtered_df is None or self.last_filtered_df.empty:
                self.show_message("결과 필요", "먼저 PDF/DXF 추출 결과를 만드세요.", error=True)
                return
            try:
        ...
        if not file_path.lower().endswith((".xls", ".xlsx")):
            self.show_message("미리보기 불가", "그래프 미리보기는 PDF/DXF/Excel 결과만 지원합니다.", error=True)
            return
    ...
    def update_inline_preview(self):
        ...
        if file_path.lower().endswith(".dxf"):
            if (
                self.dxf_profile_match_df is not None
                and not self.dxf_profile_match_df.empty
                and self.dxf_profile_source_path == file_path
            ):
                self.last_raw_preview_df = self.dxf_profile_match_df.copy()
                self.preview_source_path = file_path
                self.show_preview_dataframe(self.last_raw_preview_df, "DXF 전체 점 미리보기")
                return
            self.set_preview_message(f"DXF 미리보기 계산 중: {os.path.basename(file_path)}")
            try:
                self.extract_dxf_profile(file_path)
            except Exception as exc:
                self.set_preview_message(f"DXF 미리보기 오류: {exc}")
                self.clear_preview_plot()
                self.clear_inline_result()
            return

        if not file_path.lower().endswith((".xls", ".xlsx")):
            self.set_preview_message("인라인 미리보기는 PDF/DXF/Excel 파일을 선택했을 때 표시됩니다.")
            self.clear_preview_plot()
            self.clear_inline_result()
            return
    ...
    def apply_conversion_settings(self):
        ...
        try:
            ...
            if file_path.lower().endswith(".dxf"):
                _text_df, raw_df = process_dxf_profile(
                    file_path,
                    self.append_log,
                    y_tolerance=PDF_Y_TOLERANCE,
                    x_tolerance=PDF_X_TOLERANCE,
                ...
                if raw_df is None or raw_df.empty:
                    self.show_message("미리보기 실패", "DXF에서 전체 점 데이터를 추출하지 못했습니다.", error=True)
                    return
                self.dxf_profile_match_df = raw_df.copy()
                self.dxf_profile_source_path = file_path
                ...
                filtered_df = self._apply_distance_rounding_filter(filtered_df)
                self.show_preview_dataframe(filtered_df, "DXF 변환 설정 적용 미리보기")
                self.set_status("DXF 변환 설정 적용 완료")
                return
    ...
    def start_analysis(self):
        if not self.selected_files:
            self.show_message("파일 필요", "먼저 PDF, DXF 또는 Excel 파일을 선택하세요.", error=True)
            return
        ...
        settings = {
            ...
            "round_distance": self.round_distance_check.isChecked()
    ...
    def _set_working(self, working):
        ...
        has_dxf = any(f.lower().endswith(".dxf") for f in self.selected_files)
        ...
        self.graph_button.setDisabled(working or not (has_excel or has_dxf or has_result_data))
